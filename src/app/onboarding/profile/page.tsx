...
export default async function ProfileOnboardingPage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("display_name, birthday, onboarding_step")
    .eq("id", user.id)
    .maybeSingle();
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-5 py-10">
      ...
      <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
        Расскажите немного о себе — имя и день рождения появятся в календаре праздников.
      </p>
      <ProfileOnboardingForm displayName={profile?.display_name ?? "Пользователь"} />
    </main>
  );
}
